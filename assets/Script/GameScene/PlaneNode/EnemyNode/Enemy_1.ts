...
export default class Enemy_1 extends Plane {
	...
	update(dt) {
		// if(this.node.y <= -cc.winSize.height/2 || this.node.x < -cc.winSize.width/2 || this.node.x > cc.winSize.width/2) {
		// 	this.hit();
		// }
	}
	...
	initEnemy() {
		...
		let rangeX = maxX - minX; // 范围
		let random1 = Math.floor(Math.random() * rangeX) + minX;
		let random2 = Math.random() < 0.5 ? -1 : 1; // 坐标系
	...
	initMove() {
		// 向下移动吧
		let endPoint = cc.v2(this.node.x, -cc.winSize.height);
		// 时间
		let time = Math.abs(endPoint.y / this.enemySpeed);
		// 从 easing.ts 拿到 easing，就叫 ani 吧
		let ani: string = Easing.easingTs();
		// cc.log(ani);
		// 或许有多个缓动呢
		let tween_1 = cc.tween().to(time, {position: endPoint}, {easing: ani});
		// 执行缓动
	...
	initDropItem() {
		...
		// cc.log(this.dropItems.filter(this.dropItems))
	...
	// 			switch(this.dropItems[i].data.group) {
	// 				case "gold":
	// 					this.createDropGold(this.dropItems[i], pos);
	// 					break;
	// 				case "hp":
	// 					this.createDropHp(this.dropItems[i], pos);
	// 					break;
	// 				case "attack":
	// 					this.createDropAttack(this.dropItems[i], pos);
	// 					break;
	// 			}
	// 		}
	// 	}
			...
			if(this.Hp > 0) {
				...
				if(this.Hp <= 0){
					...
					if(this.dropItems) {
						...
						// 随机掉落物品, 不知道为什么，写成方法的形式会莫名失去 pos----->this.onCreateDrop(pos)
						let random = Math.floor(Math.random() * this.dropItems.length);
						for(let i = 0; i < this.dropItems.length; i++) {
							if(i == random) {
								switch(this.dropItems[i].data.group) {
