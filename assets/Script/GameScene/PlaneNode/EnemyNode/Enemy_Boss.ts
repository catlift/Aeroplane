...
export default class Enemy_1 extends Plane {
	...
	initMove() {
		// 向下移动吧
		let endY = cc.winSize.height/2 - this.node.height/2;
		let endPoint = cc.v2(this.node.x, endY);
		// 时间
		let time = Math.abs(endPoint.y / this.enemySpeed) / 2;
		// 从 easing.ts 拿到 easing，就叫 ani 吧
		let ani: string = Easing.easingTs();
		// 或许有多个缓动呢
		let tween_1 = cc.tween().to(time, {position: endPoint}, {easing: ani});
		let leftX = -(cc.winSize.width/2 - this.node.width/2);
		let rightX = cc.winSize.width/2 - this.node.width/2;
		let tween_2 = cc.tween().to(time * 2, {position: cc.v2(leftX, endPoint.y)}, {easing: ani}).to(time * 2, {position: cc.v2(rightX, endPoint.y)}, {easing: ani});
		// 执行缓动
